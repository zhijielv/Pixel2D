...
namespace Framework.Scripts.Utils
{
    ...
    public class MathUtility
    {
        ...
        public static void CapsuleColliderEndCaps(CapsuleCollider2D capsuleCollider, Vector3 position,
            Quaternion rotation, out Vector3 firstEndCap, out Vector3 secondEndCap)
        {
            var direction = CapsuleColliderDirection(capsuleCollider);
            var heightMultiplier = CapsuleColliderHeightMultiplier(capsuleCollider);
            var radiusMultipler = ColliderRadiusMultiplier(capsuleCollider);
            firstEndCap = TransformPoint(position, rotation,
                Vector3.Scale(capsuleCollider.bounds.center, capsuleCollider.transform.lossyScale) + direction *
                (-(capsuleCollider.size.y * heightMultiplier * 0.5f) + capsuleCollider.size.x * radiusMultipler));
            secondEndCap = firstEndCap + (rotation * direction) *
                (capsuleCollider.size.y * heightMultiplier - capsuleCollider.size.x * radiusMultipler * 2);
        }
        ...
        public static Vector3 CapsuleColliderDirection(CapsuleCollider2D capsuleCollider)
        {
            Vector3 direction;
        ...
        public static float CapsuleColliderHeightMultiplier(CapsuleCollider2D capsuleCollider)
        {
            // Use the cached transform for quick lookup.
            Transform transform;
        ...
        public static float ColliderRadiusMultiplier(Collider2D collider2D)
        {
            // Use the cached transform for quick lookup.
            Transform transform;
            ...
            {
                var capsuleCollider = collider2D as CapsuleCollider2D;
